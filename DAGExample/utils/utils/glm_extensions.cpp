...
{
	// Extended functions
	mat4 make_matrix_from_zAxis(const vec3& pos, const vec3& zAxis, const vec3& yAxis) {
		vec3 z = normalize(zAxis);
		vec3 x = normalize(cross(yAxis, z));
		vec3 y = cross(z, x);
		return mat4{ {x.x,   x.y,   x.z,   0.0f},
			         {y.x,   y.y,   y.z,   0.0f},
			         {z.x,   z.y,   z.z,   0.0f},
			         {pos.x, pos.y, pos.z, 1.0f} };
	}

	mat4 make_frustum(float left, float right, float bottom,
		...
		float top, float znear, float zfar)
	{
		...
		temp3 = top - bottom;
		temp4 = zfar - znear;
		return mat4{ /*c1*/{ temp2 / temp, 0.0f, 0.0f, 0.0f },
	...
	mat4 make_perspective_inv(float fov, float aspect_ratio, float near, float far)
	{
		float ymax = near * tanf(0.5f*radians(fov));
		float xmax = ymax * aspect_ratio;
