			...
			if (range == 1) {
				...
				// least square fit
				{
					...
					for (int i = start + laneId; i < start + range; i += 32) {
						...
						float3x3 outerProd = make_float3x3(relpos.x * relpos, relpos.y * relpos, relpos.z * relpos);
					...
					if (trace(scatterMatrix) == 0.f) {
						...
						for (int i = 0; i < 20; i++) {
							if (length(v) == 0.f) {
								v = make_float3(rand_lut[randidx++ % 64], rand_lut[randidx++ % 64],
									rand_lut[randidx++ % 64]);
								i = 0;
							}
...
)
{
  ...
  //CUDA_CHECKED_CALL cudaMemset(pColorRanges, 0, blocks.size() * sizeof(BlockBuild));
  ...
  //int jobs = int(blocks.size()) - 2; // first job is the last valid idx.
